...
//    }
     ...
     * 如果不存在则生成token并写入redis中，实效时间为10s
     *
    ...
    public Token refreshToken(String refreshToken) {
        ...
        try {
            ...
            if (lockAcquired) {
                ...
                String tokenString = this.redisTemplate.opsForValue().get(REFRESH_TOKEN_KEY + key);
                ...
                // 写入Redis，过期时间设置为10秒
                this.redisTemplate.opsForValue().set(REFRESH_TOKEN_KEY + key, JSON.toJSONString(token), Duration.ofSeconds(LOCK_SECONDS));
