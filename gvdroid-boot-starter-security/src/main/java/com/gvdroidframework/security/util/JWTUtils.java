...
public class JWTUtils {
     ...
     * @return DecodedJWT
     */
    public static DecodedJWT getDecodedJWT(String token, String secretKey) {
        return decodeToken(token, secretKey);
    }
    ...
    public static void main(String[] args) {
        String token = genToken("colin", "secretKey", 600);
...
//        String token = genToken(map, "11111", 10);
        ...
        list.forEach(x -> {
            ...
            map.put("2", "2");
            map.put("t", String.valueOf(new Date().getTime() + 10));
...
//        String[] s = list2.stream().findFirst().get();
        ...
        list2.forEach(x -> {
            String token = x[1];
//            1
            Map<String, String> m = getMap(x[1], x[0], "user", "channel", "entity", "2", "t");


            long t = Long.parseLong(m.get("t"));
